...
static int reset(sensor_t *sensor)
{
    ...
    // Reset all registers
    i2c_write_bit(sensor->slv_addr, COM7, 7, 1);
    ...
    // {
    //     i2c_write_bit(sensor->slv_addr, regs[i][0], regs[i][1]);
    // }
...
int ov7670_init(sensor_t *sensor)
{
    ...
    sensor->id.MIDH = i2c_read_reg(sensor->slv_addr, REG_MIDH);
    sensor->id.MIDL = i2c_read_reg(sensor->slv_addr, REG_MIDL);
    sensor->id.PID  = i2c_read_reg(sensor->slv_addr, REG_PID);
    sensor->id.VER  = i2c_read_reg(sensor->slv_addr, REG_VER);
